...
fn parse_expression(tokens: &mut VecDeque<Token>) -> Option<Expression> {
	...
	let initial = match tok {
		...
		_ => panic!("Expected expression-initial token at code starting {:?}", tokens)
	};
	...
	match tokens.front() {
		...
		Some(&Token::Star) |
		Some(&Token::Slash) => {
			let got = tokens.pop_front().expect("Got an op that doesn't make sense, I guess");
			Some(Expression::Binary {
				op: got,
				lhs: Box::new(initial),
				rhs: Box::new(parse_expression(tokens).expect("Binary expression expected a right operand")),
			})
		},
	...
	match next {
		Some(Token::LeftParen) => {
			...
			while(tokens.front() != Some(&Token::RightParen)) {
				p.push(Box::new(parse_expression(tokens).expect("Parsing parameter didn't yield valid expression")));
			}
			p.reverse();
...
fn parse_return(tokens: &mut VecDeque<Token>) -> Option<Box<Statement>> {
    ...
    Some(Box::new(Statement::Return(parse_expression(tokens).expect("Return statements need things to return"))))
}
